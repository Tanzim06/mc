...
//    List<ChargeGroupData> findAllGroup(@Param("chargeGroupName") String chargeGroupName, @Param("remarks") String remarks,@Param("chargeGroupId") Long chargeGroupId);
   ...
   // Long id, String chargeGroupName, int activeStatus, Long chargeGroupId, Long itemId, Double itemRate, Long currencyId, String remarks, String itemName
    ...
    @Query("SELECT new com.bz.mc.facade.data.ChargeGroupData(b.id,a.chargeGroupName,b.activeStatus,a.origin,b.chargeGroupId,b.itemId,b.itemRate,b.currencyId,a.remarks,i.itemName)" +
            " FROM ChargeGroup a inner join ChargeItem b on a.id = b.chargeGroupId inner join ItemInfo i on b.itemId = i.id " +
            " where  a.chargeGroupName LIKE CONCAT('%',:chargeGroupName, '%') and  a.remarks LIKE CONCAT('%',:remarks, '%') and a.origin = coalesce(:origin,a.origin) ")
    List<ChargeGroupData> findAllChargeList(@Param("chargeGroupName") String chargeGroupName, @Param("remarks") String remarks, @Param("origin") Origin origin);


    @Query("SELECT new com.bz.mc.facade.data.ChargeGroupData(a.id,a.chargeGroupName,a.shortCode,a.sessionId,a.programId, a.programSegmentId,a.chargeCycle,a.paymentCycle,b.activeStatus,a.effectiveFrom,a.effectiveTo,a.origin,b.chargeGroupId,b.itemId,b.itemRate,b.currencyId,a.remarks,i.itemName,s.sessionName,p.programName,ps.segmentName,bd.billDate)" +
            " FROM ChargeGroup a inner join ChargeItem b on a.id = b.chargeGroupId inner join ItemInfo i on b.itemId = i.id inner join SessionInfo s on   a.sessionId = s.id inner join Program p on a.programId = p.id inner join ProgramSegment ps on a.programSegmentId = ps.id inner join BillDate bd on a.id = bd.chargeGroupId " +
